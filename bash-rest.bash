...
build_endpoint_handler_function() {
	...
	local controller_switch_end='*) bash_rest_http_response="HTTP/1.1 404 NotFound\r\n\r\n\r\nNot Found ${bash_rest_incoming_request_http_method_and_endpoint}";; esac '

	for i in "${!endpoint_function_map[@]}"; do
		controller_switch_statement+="\"$(strip_path_variable_names "${i}")\") bash_rest_response_body=\"\$(${endpoint_function_map[${i}]} \"\${bash_rest_endpoint_path_variables_array[@]}\")\";; "
	done
	...
	{ source /dev/fd/999; } 999<<DECLARE_BASH_REST_ENDPOINT_HANDLER_FUNCTION
		bash_rest_endpoint_handler() {
				  ...
				  local bash_rest_response_body
				  local bash_rest_http_response
